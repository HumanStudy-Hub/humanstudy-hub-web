...
export default async function PlaygroundPage({ searchParams }: { searchParams: Promise<{ mode?: string; study?: string }> }) {
  const { mode, study } = await searchParams;
  ...
  if (mode === "reproduce") return <PlaygroundStudio studies={options} initialStudyId={study} />;
